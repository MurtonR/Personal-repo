...
int servoEyePin = 9;  //servo for vertical movement
int servoY2Pin = 5;
...
void RobotHead(uint8_t sensed){
   ...
   switch (currentState){
    ...
    case THINKING: //state for time expensive code such as what audio to play
...
void getCoordinates(char bytes){  //runs for every byte in the serial communication, outputs whole coordinates
   ...
   if (bytes == 'n') { //checks to see if endline character is found
      ...
      if (seperator != -1) { // runs if comma is present 
          ...
          faceXmove = map(faceX, 540, -70, 0,160 );  //maps x and y to the servo range
          faceYmove = map(faceY, 360, 30, 180,0 );
...
//data from python needs to be organised like this ___,___n    where the blanks are the x and y values, the n is there to end the communication
  if (Serial.available() > 0 && LDRValue > 100 && throwing == false){
  ...
  if( millis() - timer> 5000){
    move(0, eye);
    currentPositionEye = 0;
    smoothMove(180, eye, currentPositionEye);
    currentPositionEye = 180;
    smoothMove(0, eye , currentPositionEye);
    
  }
  /*  Only uncomment this is an LDR is connected, the code will not function otherwise

  if (Serial.available() > 0 && LDRValue < 600){ //lower value == less light == covered by hat
    visualSensor = HAT;
  }
