...
//initalise state machines
enum states {IDLE, FOLLOW, THINKING, THROW};
enum vision {VOID, DETECTED, OBSERVED, HAT};

//initialise a version of each state
static states currentState;  
static vision visualSensor;  

//servo for eye movement
int servoEyePin = 9;  
...
//initialise variable to hold current position of each servo 
int currentPositionEye;
int currentPositionZ; 
int currentPositionY; 

//initalise variables for looping
int i;
int j;
...
//Variables for processing the string from python
String inputString; 
...
//data from python needs to be organised like this ___,___n    where the blanks are the x and y values, the n is there to end the communication
  ...
  else if (Serial.available() > 0 && LDRValue < 100 && throwing == false){ //When serial available, LDR is covered and the hat is not currently being thrown off
    bytes = Serial.read(); //set byte from serial communication
    timer = millis(); //set timer to determine the time since last tracking
    visualSensor = HAT;
  }
  ...
  else if( millis() - timer> 5000){ //After 5 seconds run idle movement of eyes
